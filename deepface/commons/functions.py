...
def loadBase64Img(uri):
    """이미지64 로드 함수 Load image from base64 string.
...
def load_image(img):
    """이미지 로드 함수 -> 결국 cv2.imread('img')로 읽음
...
def extract_faces(
    ...
    img = load_image(img)  # cv2.imread로 읽은 값
    img_region = [0, 0, img.shape[1], img.shape[0]]  # 그것의 [0, 0, x, y] 크기
    ...
    for current_img, current_region, confidence in face_objs:
        if current_img.shape[0] > 0 and current_img.shape[1] > 0:
            ...
            # resize and padding (크기 조절하고 패딩 넣는 코드)
            if current_img.shape[0] > 0 and current_img.shape[1] > 0:
